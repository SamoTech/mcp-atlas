  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-16">
      <div className="mb-12">
        ...
        <p className="text-slate-400">Live data across {cases.length} verified enterprise MCP deployments.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
        {[
          ...
          <div key={k.label} className="rounded-xl border border-slate-700 bg-slate-900 p-5 text-center">
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-8">
        <div className="rounded-xl border border-slate-700 bg-slate-900 p-6">
          ...
          <div className="space-y-3">
            {([['high', 'bg-emerald-500', 'text-emerald-300'], ['medium', 'bg-yellow-500', 'text-yellow-300'], ['low', 'bg-red-500', 'text-red-300']] as const).map(([level, bar, text]) => {
        ...
        <div className="rounded-xl border border-slate-700 bg-slate-900 p-6">
          ...
          <div className="space-y-3">
            {Object.entries(patternCounts).sort((a, b) => b[1] - a[1]).map(([pattern, count]) => {
              ...
              const color = patternColors[pattern] || 'bg-slate-600'
              return (
                <div key={pattern}>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-slate-200 capitalize">{pattern.replace(/-/g, ' ')}</span>
      ...
      <div className="rounded-xl border border-slate-700 bg-slate-900 p-6 mb-8">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {Object.entries(industryCounts).sort((a, b) => b[1] - a[1]).map(([industry, count]) => {
            ...
            const color = industryColors[industry] || 'bg-slate-600'
            return (
              <div key={industry}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-slate-200 capitalize">{industry.replace(/-/g, ' ')}</span>
                  <span className="text-slate-400">{count}</span>
                </div>
                <div className="h-2 rounded-full bg-slate-800">
                  <div className={`h-2 rounded-full ${color} transition-all`} style={{ width: `${pct}%` }} />
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="rounded-xl border border-slate-700 bg-slate-900 p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">{withGovernance} of {cases.length} cases document access controls</p>
          </div>
        </div>
        <div className="rounded-xl border border-slate-700 bg-slate-900 p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">{withOutcomes} of {cases.length} cases include measurable results</p>
          </div>
        </div>
      </div>
    </div>
